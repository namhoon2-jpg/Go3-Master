...
if excel_file and pdf_file and target_major:
    if not st.session_state.analysis_result:
        with st.spinner('🚀 입시 전문가 AI가 학생부를 정밀 분석 중입니다...'):
            ...
            # 💡 핵심 1: 대왕 폰트 유발자 마크다운 샵(#) 원천 사용 금지령 추가
            prompt = f"""
            ...
            4. **[대왕 폰트 방지 (마크다운 금지)]**: 글씨 크기가 비정상적으로 커지는 오류를 막기 위해, 내용 중에 샵('#') 기호를 활용한 제목 표시(예: # 질문1, ## 주제 등)를 절대 쓰지 마세요.
            5. **마지막 두 줄 차트 태그 규칙**: 아래 지정된 2줄의 태그만 정확히 출력할 것. 태그 옆이나 아래에 '(합계 100)'이나 '(0~100)' 같은 설명 텍스트를 절대로 덧붙이지 마세요. (숫자만 치환할 것)
               @PIE [교과: X, 종합: Y, 정시: Z] @
    ...
    # 💡 핵심 2: 혹시라도 AI가 뱉어낸 샵 기호(#)를 물리적으로 완전 멸균 처리 (대왕 폰트 100% 방어)
    ...
    # 가시성 강화 및 줄바꿈 해결
    p2 = re.sub(r'(?i)(?:-\s*)?농어촌\s*전형\s*전략|(?:-\s*)?농어촌\s*전형\s*유불리\s*판단', '\n\n⚖️ **농어촌 전형 전략**', p2)
    p2 = re.sub(r'(?i)(?:-\s*)?생기부\s*보완\s*전략', '\n\n🛠️ **생기부 보완 전략**', p2)

    p3 = re.sub(r'(?i)(?:-\s*)?주제\s*:', '\n\n📍 **주제:** ', p3)
    p3 = re.sub(r'(?i)(?:-\s*)?종적/횡적\s*근거\s*:', '\n\n🔍 **종적/횡적 근거:** ', p3)
    p3 = re.sub(r'(?i)(?:-\s*)?탐구\s*방법\s*:', '\n\n🛠️ **탐구 방법:** ', p3)
    p3 = re.sub(r'(?i)(?:-\s*)?세특\s*예시\s*:', '\n\n✍️ **세특 예시:** ', p3)
    
    p4 = re.sub(r'(?i)(?:-\s*)?질문\s*:', '\n\n❓ **질문:** ', p4)
    p4 = re.sub(r'(?i)(?:-\s*)?모범\s*답안\s*:', '\n\n✅ **모범 답안:** ', p4)
    p4 = re.sub(r'(?i)(?:-\s*)?준비\s*방법\s*:', '\n\n🛠️ **준비 방법:** ', p4)
